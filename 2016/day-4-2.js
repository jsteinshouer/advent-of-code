...
//Test input: first three are real
// const input = [
...
const shiftCipher = function(letter,shift) {
	let index = abc.indexOf(letter);
	let newIndex = index + (shift % 26);

	//if (increment > 25) increment = 26 - increment;
	//console.log(shift % 26);
	//console.log(index);
...
input.forEach(function(item) {
	...
	/* Get the sector and checksum */
	let sectorPlusChecksum = list.pop().replace("]","").split("[");
	let sector = parseInt(sectorPlusChecksum[0],10);
	let checksum = sectorPlusChecksum[1];
	let encodedName = list.join("");
	let counts = _.countBy( encodedName );
	counts = _.map(counts,function(val,key) {
		return {"char": key, "count": val};
	});
	counts = _.orderBy(counts,["count","char"],["desc","asc"]);
	
	let check = _.map(counts,function(val,i) {
		if (i < 5 )return val.char;
	}).join("");
	
	if (check == checksum) {
		let roomName = _.map(item.replace(item.split("-").pop(),""),function(val) {
			if (val == "-") return " ";
			return shiftCipher(val,sector);
		}).join("");

		if (roomName.match(/north/g)) {
			console.log(sector);
		}
	}
