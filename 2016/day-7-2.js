...
const getABA = function(str) {
	//return str.match(/(?:([a-z])([a-z])(?!\2)\1)*/g);
	...
	chars.forEach(function(char) {
		if (chars[index] && chars[index+1] && chars[index+2]) {
			if (chars[index] == chars[index+2] && chars[index+1] != chars[index]) {
				let newABA = chars[index] + chars[index+1] + chars[index+2];
				if (aba.indexOf(newABA) === -1) aba.push(newABA);
			}
		}
		index = index + 1;
	});
...
input.forEach(function(address) {
	let supernet = address.split(/\[[a-z]+\]/);
	let hypernet = address.match(/\[([a-z]+)\]/g);	

	let supernetABA = [];
	let hypernetBAB = [];

	supernet.forEach(function(item) {
		supernetABA = _.compact(_.concat(supernetABA,getABA(item.replace("[","").replace("]",""))));
	});

	hypernet.forEach(function(item) {
		hypernetBAB = _.compact(_.concat(hypernetBAB,getABA(item)));
	});

	let supportSSL = false;
	supernetABA.forEach(function(item) {
		let char = item.split("");
		let bab = (char[1] + char[0] + char[1]).toString();
		if (hypernetBAB.indexOf(bab) > -1) supportSSL = true;
	});
