...
const containsABBA = function(str) {
	var match = str.match(/([a-z])([a-z])(?!\1)\2\1/);
...
input.forEach(function(address) {
	let segments = address.split(/\[[a-z]+\]/);
	let hypernet = address.match(/\[([a-z]+)\]/g);
	//console.log(address);
	// console.log(hypernet);
	let hypernetHasABBA = false;
	let segmentsHasABBA = false;

	hypernet.forEach(function(item) {
		if (containsABBA(item)) {
	...
	if (!hypernetHasABBA) {
		segments.forEach(function(item) {
			if (!segmentsHasABBA && containsABBA(item)) {
				segmentsHasABBA = true;
			}
		});
	}
	if (!hypernetHasABBA && segmentsHasABBA) {
		count = count + 1;
	}
});
